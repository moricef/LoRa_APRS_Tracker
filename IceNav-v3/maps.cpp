...
extern Gps mapsGps; // Use the renamed instance to avoid conflict with TinyGPSPlus gps
extern Storage storage;
extern std::vector<wayPoint> trackData; /**< Vector containing track waypoints */
// extern Configuration Config; // Needs to be extern to access mapSet
 ...
 */
Maps::Maps() : fillPolygons(true), mapConfig(Config) // Pass global Config reference for mapSet access
{
    // Initialize static members that need default values or sizing
    #ifdef BOARD_HAS_PSRAM
        size_t psramFree = ESP.getFreePsram();
        // Allocate 4.5MB for tiles (36 tiles * 128KB) or adjust based on available PSRAM
        maxCachedTiles = std::min(static_cast<size_t>(36), psramFree / (Maps::TILE_SIZE_PUBLIC * Maps::TILE_SIZE_PUBLIC * 2)); // Use TILE_SIZE_PUBLIC for calculation
        maxUnifiedPoolEntries = std::min(static_cast<size_t>(100), psramFree / (1024 * 32)); // ~32KB per entry for other dynamic allocations
    #else
        size_t ramFree = ESP.getFreeHeap();
        maxCachedTiles = std::min(static_cast<size_t>(9), ramFree / (Maps::TILE_SIZE_PUBLIC * Maps::TILE_SIZE_PUBLIC * 2)); // Use TILE_SIZE_PUBLIC for calculation
        maxUnifiedPoolEntries = std::min(static_cast<size_t>(25), ramFree / (1024 * 64)); // Larger chunk for RAM
    #endif

    ESP_LOGI(TAG, "Maps constructor completed. Max cached tiles: %zu, Max pool entries: %zu", maxCachedTiles, maxUnifiedPoolEntries);
}

/**
 * @brief Sets the center coordinates of the map.
 * @param lat Latitude of the new center.
 * @param lon Longitude of the new center.
 */
void Maps::setCenter(float lat, float lon) {
    currentMapTile.lat = lat;
    currentMapTile.lon = lon;
}
 ...
 */
void Maps::setZoom(uint8_t zoom) {
    zoomLevel = zoom;
}
...
void Maps::generateMap(uint8_t zoom)
{
        ...
        Maps::currentMapTile.tiley != Maps::oldMapTile.tiley)
    {
        ...
        // Use mapConfig.mapSet.vectorMap to decide rendering mode
        if (mapConfig.mapSet.vectorMap)
...
void Maps::displayMap()
{
    ...
    mapHeading = mapConfig.mapSet.mapRotationComp ? compass.getHeading() : mapsGps.gpsData.heading;
#else
    mapHeading = mapsGps.gpsData.heading;
#endif
    ...
    if (Maps::followGps)
    {
        const float lat = mapsGps.gpsData.latitude;
        const float lon = mapsGps.gpsData.longitude;
...
bool Maps::renderTile(const char* path, const int16_t xOffset, const int16_t yOffset, TFT_eSprite &map)
{
    ...
    // Helper functions to mimic old latLonToTile signature for queueAdjacentZooms
    // Implementation outside the class for static members.
    void Maps::lon2tile(float lon, int z, int* tx) { *tx = Maps::lon2tilex(lon, z); }
    void Maps::lat2tile(float lat, int z, int* ty) { *ty = Maps::lat2tiley(lat, z); }
